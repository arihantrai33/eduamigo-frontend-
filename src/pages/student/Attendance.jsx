...
export default function StudentAttendance() {
  ...
  return (
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", background: "#f5f6fa", fontFamily: "Inter, sans-serif" }}>
      ...
      {/* Header */}
      <div style={{ background: "linear-gradient(135deg, #1a73e8 0%, #6c63ff 100%)", padding: "48px 20px 24px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
          <button onClick={() => navigate(-1)}
            style={{ background: "rgba(255,255,255,0.2)", border: "none", borderRadius: "50%", width: 36, height: 36, cursor: "pointer", color: "#fff", fontSize: 18 }}>
            ←
          </button>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 80px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 20 }}>
          {[
            ...
            <div key={s.label} style={{ background: s.bg, borderRadius: 14, padding: "14px 8px", textAlign: "center" }}>
        ...
        <div style={{ background: "white", borderRadius: 14, padding: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.06)", marginBottom: 16 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: "#111", marginBottom: 8 }}>Attendance Status</div>
          ...
          {pct < 75 && (
            <div style={{ marginTop: 10, background: "#fff5f5", borderRadius: 10, padding: "10px 12px", fontSize: 12, color: "#ef4444" }}>
              Minimum 75% attendance required. You need {Math.max(0, Math.ceil((0.75 * total - present) / 0.25))} more present days to reach 75%.
            </div>
          )}
        </div>
        ...
        {/* No data */}
        {total === 0 && (
          <div style={{ background: "white", borderRadius: 14, padding: 24, textAlign: "center", boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
            <div style={{ fontSize: 32, marginBottom: 8 }}>📭</div>
            <div style={{ fontSize: 14, color: "#999" }}>No attendance records yet</div>
            <div style={{ fontSize: 12, color: "#bbb", marginTop: 4 }}>Teacher ke mark karne ke baad yahan dikhega</div>
          </div>
        )}
      </div>
