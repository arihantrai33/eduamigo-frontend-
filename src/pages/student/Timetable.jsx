...
const STATUS_STYLE = {
  ...
  default:  { bg: "#f5f5f5", color: "var(--subtext)" },
};
...
export default function Timetable() {
  ...
  return (
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", background: "var(--bg)", fontFamily: "sans-serif" }}>
      {/* Header */}
      <div style={{ background: "var(--card)", padding: "48px 20px 0px", boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "16px" }}>
          <button onClick={() => navigate("/student/home")}
            style={{ background: "none", border: "none", fontSize: "20px", cursor: "pointer" }}>←</button>
          <div style={{ fontSize: "18px", fontWeight: "800", color: "var(--text)" }}>Timetable</div>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingBottom: "12px", paddingTop: "4px" }}>
          ...
          <span style={{ fontSize: "12px", color: "var(--subtext)", fontWeight: "600" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px" }}>
        <div style={{ fontSize: "11px", fontWeight: "700", color: "var(--subtext)", marginBottom: "12px", letterSpacing: "1px" }}>
          {getDateForDay(selectedDay, weekOffset)}
        </div>

        {loading ? (
          <div style={{ textAlign: "center", padding: "40px 0", color: "var(--subtext)", fontSize: 14 }}>Loading...</div>
        ) : classes.length === 0 ? (
          <div style={{ background: "var(--card)", borderRadius: "16px", padding: "40px 20px", textAlign: "center", boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
            <div style={{ fontSize: 40, marginBottom: 12 }}>📭</div>
            <div style={{ fontSize: 15, fontWeight: 700, color: "var(--text)" }}>No classes scheduled</div>
            <div style={{ fontSize: 13, color: "var(--subtext)", marginTop: 4 }}>No timetable found for {DAY_FULL[selectedDay]}</div>
          </div>
        ) : (
          <div style={{ background: "var(--card)", borderRadius: "16px", padding: "4px 16px", boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
            {classes.map((cls, i) => {
