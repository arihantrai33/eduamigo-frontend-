    ...
    <div style={{ minHeight: "100vh", background: "#f5f6fa", paddingBottom: "80px" }}>
      ...
      <div style={{ padding: "16px", marginTop: "-20px" }}>
        {notifications.length === 0 ? (
          ...
          notifications.map(n => {
            const isRead = n.read?.[user.uid] === true;
            return (
              <div key={n.id} onClick={() => markRead(n.id)}
                style={{ background: isRead ? "white" : "#eff6ff", borderRadius: "12px", padding: "14px", marginBottom: "10px", boxShadow: "0 1px 4px rgba(0,0,0,0.08)", borderLeft: isRead ? "none" : "3px solid #4f46e5", cursor: "pointer" }}>
                <div style={{ display: "flex", gap: "12px", alignItems: "flex-start" }}>
                  <span style={{ fontSize: "24px" }}>{icons[n.type] || "📢"}</span>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: isRead ? "500" : "700", fontSize: "14px", color: "#111" }}>{n.title}</div>
                  ...
                  {!isRead && <div style={{ width: "8px", height: "8px", borderRadius: "50%", background: "#4f46e5", marginTop: "4px" }} />}
                </div>
              </div>
            );
          })
