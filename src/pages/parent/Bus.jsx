...
setDriverToken(bus.firebasePath); // ✅ firebaseKey use karo
// ✅ Student name directly students API se fetch karo
try {
  const studentId = bus.assignedStudents?.[0];
  if (studentId) {
    const sRes = await fetch(`http://localhost:8000/api/students/${studentId}`, {
    ...
    setStudentData({
      ...
      class: s.class ? `Class ${s.class}` : (s.grade ? `Class ${s.grade}` : '—')
    });
  } else {
    setStudentData({ name: 'Your Child', class: '—' });
  }
} catch(e) {
  console.error('[Student fetch]', e);
  setStudentData({ name: 'Your Child', class: '—' });
}
...
// ✅ Firebase listener — firebaseKey se path banta hai
useEffect(() => {
if (!driverToken) return;
const busRef = ref(db, `${driverToken}/location`); // ✅ firebaseKey already "transport/BUS42-0k94ZU" hai
...
{isCompleted?`Departed ${getDepartureTime(i)}`:isCurrent?"🚌 Bus is here now":isSchool?`🏁 Est. arrival — ${eta}`:"Upcoming stop"}
</p>
</div>
