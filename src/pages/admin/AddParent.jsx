...
const AddParent = () => {
  ...
  const emptyForm = { name: "", email: "", phone: "", gender: "", address: "", occupation: "", children: [] };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      showToast(
        `Parent added! Login: ${res.data.loginInfo.email} / ${res.data.loginInfo.defaultPassword}`,
        "success"
      );
      setForm(emptyForm);
      setSearchQuery("");
  ...
  return (
    <div style={{ fontFamily: "Inter, sans-serif" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8, maxHeight: 300, overflowY: "auto" }}>
          ...
          {!studentsLoading && filteredStudents.length === 0 && (
            <div style={{ textAlign: "center", color: "#ccc", padding: "20px 0", fontSize: 13 }}>
              {searchQuery ? "No students match your search" : "No students found in the system"}
            </div>
          )}
      ...
      <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
        <button
          onClick={() => { setForm(emptyForm); setSearchQuery(""); }}
          style={{
            padding: "8px 20px", borderRadius: 8, border: "0.5px solid #E8E8E5",
            background: "#F5F5F3", fontSize: 12, color: "#666", cursor: "pointer",
          }}>
          Clear
        </button>
        <button
